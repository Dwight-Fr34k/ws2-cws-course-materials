window.addEventListener('load', function()
{
    ...
    for(var i = 1, len = mainSections.length; i < len ; i++)
    {
        ...
        var mainSectionTitle = mainSection.hasAttribute('data-toc-title') ? mainSection.getAttribute('data-toc-title') : undefined;
        ...
        if(subSections.length)
        {
            ...
            for(var j = 1; j < subSections.length; j++)
            {
                ...
                var subSectionTitle = subSections[j].hasAttribute('data-toc-title') ? subSections[j].getAttribute('data-toc-title') : undefined;
    ...
    for(var i = 0, len = mainSections.length; i < len; i++)
    {
        ...
        if(i == 0)
        {
            comments[mainSection.id] =
            {
                title: '<h1>' + (mainSection.querySelector('#' + mainSection.id + ' > h1') ? mainSection.querySelector('#' + mainSection.id + ' > h1').innerHTML : '(no title)') + '</h1>',
                comments: mainSection.querySelector('#' + mainSection.id + ' > .comments') ? mainSection.querySelector('#' + mainSection.id + ' > .comments').innerHTML : '&mdash;'
            }
        }
        ...
        if(subSections.length) {
            for(var j = 1; j <= subSections.length; j++) {
                // Get shorthand to section element
                var subSection = subSections[j - 1];

                // Give secion an id
                if(!subSection.id) subSection.id = 'slide_' + i + '_' + j;

                // Get subsection comment
                comments[subSection.id] =
                {
                    title: '<h2>' + (subSection.querySelector('#' + subSection.id + ' > h2') ? subSection.querySelector('#' + subSection.id + ' > h2').innerHTML : '(no title)') + '</h2>',
                    comments: subSection.querySelector('#' + subSection.id + ' > .comments') ? subSection.querySelector('#' + subSection.id + ' > .comments').innerHTML : '&mdash;'
                }
            }
        }
    }

    window.buildPopupComments = function()
    {
        for (var i in comments)
        {
            var commentsHtml = '<div data-slide-id="' + i + '" id="' + i + '">' + comments[i].title + comments[i].comments + '</div>';
            commentsPopup.document.body.innerHTML = commentsPopup.document.body.innerHTML + commentsHtml;
        }
    }

    var commentsPopup = undefined;
    document.addEventListener('keydown', function(e)
    {
        var evtobj = window.event? event : e;
        if(evtobj.keyCode == 32 && evtobj.ctrlKey)
        {
            commentsPopup = window.open('comments.html', 'comPop', 'directories=no,location=no,menubar=no,status=no,toolbar=no,width=400,height=800');
            e.preventDefault();
            e.stopPropagation();
        }
        if(commentsPopup && commentsPopup != undefined && commentsPopup.highlightComment) commentsPopup.highlightComment(Reveal.getCurrentSlide().id);
    })

    window.addEventListener('unload', function()
    {
        if(commentsPopup != undefined) commentsPopup.close();
    });
